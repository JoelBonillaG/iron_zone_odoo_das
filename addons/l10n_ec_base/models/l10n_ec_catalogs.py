...
class L10nEcContributorType(models.Model):
    ...
    retention_income_id = fields.Many2one(
        'l10n_ec.retention.code',
        string='Retención Renta',
        domain=[('type', '=', 'ir')],
    )
    retention_vat_goods_id = fields.Many2one(
        'l10n_ec.retention.code',
        string='Retención IVA Bienes',
        domain=[('type', '=', 'iva'), ('applies_to', 'in', ('goods', 'both'))],
    )
    retention_vat_services_id = fields.Many2one(
        'l10n_ec.retention.code',
        string='Retención de IVA de servicios',
        domain=[('type', '=', 'iva'), ('applies_to', 'in', ('services', 'both'))],
    )
    active = fields.Boolean(default=True)
