...
class ResPartner(models.Model):
    ...
    l10n_ec_contributor_type_id = fields.Many2one(
        ...
        string="SRI Tipo de Contribuyente",
        help="Clasificación SRI usada para ATS y reglas de retención.",
    )

    l10n_ec_related_party = fields.Boolean("Related Party (ATS)", default=False)

    @api.onchange("l10n_latam_identification_type_id", "vat", "country_id")
    def _onchange_l10n_ec_identification_type(self):
        self._sync_l10n_ec_identifier_type()

    @api.model_create_multi
    def create(self, vals_list):
        records = super().create(vals_list)
        records._sync_l10n_ec_identifier_type()
        return records

    def write(self, vals):
        result = super().write(vals)
        if {"l10n_latam_identification_type_id", "vat", "country_id"} & set(vals):
            self._sync_l10n_ec_identifier_type()
        return result

    def _sync_l10n_ec_identifier_type(self):
        for partner in self:
            ec_identifier_type = partner._get_l10n_ec_identifier_type()
            if partner.l10n_ec_identifier_type != ec_identifier_type:
                partner.l10n_ec_identifier_type = ec_identifier_type

    def _get_l10n_ec_identifier_type(self):
        self.ensure_one()
        ...
        if self.l10n_latam_identification_type_id:
            xmlid = self.l10n_latam_identification_type_id.get_external_id().get(
                self.l10n_latam_identification_type_id.id
            )

        if xmlid == "l10n_ec.ec_ruc":
            return "ruc"
        if xmlid == "l10n_ec.ec_dni":
            return "cedula"
        if xmlid == "l10n_ec.ec_passport":
            return "pasaporte"

        vat = (self.vat or "").strip()
        if vat.isdigit() and len(vat) == 13:
            return "ruc"
    ...
    @api.constrains("vat", "l10n_ec_identifier_type", "l10n_latam_identification_type_id", "country_id")
            ...
            if result.get("success"):
                ...
                if data.get("regimen_rimpe"):
                    if "EMPRENDEDOR" in data.get("regimen_rimpe", "").upper():
                        ...
                        self.l10n_ec_contributor_type_id = self.env.ref(
                            "l10n_ec_base.contrib_rimpe_emprendedor", raise_if_not_found=False
                        )
                    elif "POPULAR" in data.get("regimen_rimpe", "").upper():
                        self.l10n_ec_taxpayer_type = "rimpe_p"
                        self.l10n_ec_contributor_type_id = self.env.ref(
                            "l10n_ec_base.contrib_rimpe_negocio", raise_if_not_found=False
                        )
                elif data.get("contribuyente_especial"):
                    self.l10n_ec_taxpayer_type = "special"
                    self.l10n_ec_contributor_type_id = self.env.ref(
                        "l10n_ec_base.contrib_especial", raise_if_not_found=False
                    )
                else:
                    self.l10n_ec_taxpayer_type = "general"
                    self.l10n_ec_contributor_type_id = self.env.ref(
                        "l10n_ec_base.contrib_persona_natural", raise_if_not_found=False
                    )
    ...
    def action_load_from_sri(self):
        ...
        if data.get("regimen_rimpe"):
            if "EMPRENDEDOR" in data.get("regimen_rimpe", "").upper():
                vals["l10n_ec_taxpayer_type"] = "rimpe_e"
                contributor_type = self.env.ref(
                    "l10n_ec_base.contrib_rimpe_emprendedor", raise_if_not_found=False
                )
            elif "POPULAR" in data.get("regimen_rimpe", "").upper():
                vals["l10n_ec_taxpayer_type"] = "rimpe_p"
                contributor_type = self.env.ref(
                    "l10n_ec_base.contrib_rimpe_negocio", raise_if_not_found=False
                )
            else:
            ...
            vals["l10n_ec_taxpayer_type"] = "special"
            contributor_type = self.env.ref(
                "l10n_ec_base.contrib_especial", raise_if_not_found=False
            )
        else:
            vals["l10n_ec_taxpayer_type"] = "general"
            contributor_type = self.env.ref(
                "l10n_ec_base.contrib_persona_natural", raise_if_not_found=False
            )

        if contributor_type:
            vals["l10n_ec_contributor_type_id"] = contributor_type.id
