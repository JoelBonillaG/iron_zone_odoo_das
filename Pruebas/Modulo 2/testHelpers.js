...
export const TRAINER_USER = {
  name: process.env.IRONZONE_TRAINER_NAME || "Carlos Mendez",
  email: process.env.IRONZONE_TRAINER_EMAIL || "carlos.mendez@ironzone.ec",
  password: process.env.IRONZONE_TRAINER_PASSWORD || "admin123",
  phone: process.env.IRONZONE_TRAINER_PHONE || "0991234502",
};

export const ADMIN_USER = {
  name: process.env.IRONZONE_ADMIN_NAME || "Administrador",
  email: process.env.IRONZONE_ADMIN_EMAIL || "admin",
  password: process.env.IRONZONE_ADMIN_PASSWORD || "admin",
};

export const LOCAL_BASE_URL =
  process.env.IRONZONE_LOCAL_URL || "http://localhost:8069";
export const DEPLOYED_BASE_URL =
  process.env.IRONZONE_DEPLOYED_URL || "https://iron-zone.stratiumhub.com";

const moduleDir = path.dirname(fileURLToPath(import.meta.url));
const evidenceDir = path.join(moduleDir, "evidencias");
const videoDir = path.join(evidenceDir, "videos");
const videoPauseMs = Number(process.env.VIDEO_PAUSE_MS || 900);
const videoSlowMoMs = Number(process.env.VIDEO_SLOWMO_MS || 150);
...
export async function tomarCaptura(page, nombreFase) {
  const ruta = path.join(
    ensureEvidenceDir(),
    `${Date.now()}_${nombreFase}.png`,
  );
  try {
    await page.screenshot({ path: ruta, fullPage: false });
    console.log(`Captura guardada: ${ruta}`);
    await delay(videoPauseMs);
  } catch (error) {
    console.log(
      `Advertencia: no se pudo guardar captura de ${nombreFase} (${error.message})`,
    );
  }
}

async function showVideoTitle(page, title) {
  await page.setContent(`
    ...
    `);
  await delay(Number(process.env.VIDEO_TITLE_MS || 1800));
}
...
export async function runPlaywrightFlow(nombre, testFn) {
  ...
  const browser = await chromium.launch({ headless, slowMo: videoSlowMoMs });
  ...
  try {
    await showVideoTitle(page, nombre);
    ...
    if (video) {
      try {
        ...
        const finalVideoPath = path.join(
          videoDir,
          `${Date.now()}_${safeFileName(nombre)}.webm`,
        );
        fs.renameSync(originalVideoPath, finalVideoPath);
        console.log(`Video guardado: ${finalVideoPath}`);
      } catch (error) {
        console.log(
          `Advertencia: no se pudo guardar video de ${nombre} (${error.message})`,
        );
      }
    }
    await browser.close();
...
export async function loginAs(page, baseUrl, user) {
  console.log(`Iniciando sesion con ${user.email}...`);
  await page.goto(buildUrl(baseUrl, "/web/login"), {
  ...
  await Promise.all([
    page
      .waitForURL((url) => !url.href.includes("/web/login"), { timeout: 60000 })
      .catch(() => null),
    page
      .locator(
        ".oe_login_form button[type='submit'], button[type='submit'].btn-primary",
      )
      .first()
      .click(),
  ]);
  ...
  if (page.url().includes("/web/login")) {
    ...
    throw new Error(
      `No se pudo iniciar sesion con ${user.email}. Texto visible: ${text.slice(0, 300)}`,
    );
  }
...
export async function login(page, baseUrl) {
  await loginAs(page, baseUrl, TEST_USER);
}

export async function logout(page, baseUrl) {
  await page.goto(buildUrl(baseUrl, "/web/session/logout"), {
    waitUntil: "domcontentloaded",
    timeout: 60000,
  });
  await delay(videoPauseMs);
...
export async function callOdoo(
  ...
  const url = buildUrl(baseUrl, `/web/dataset/call_kw/${model}/${method}`);
  ...
  if (response.error) {
    ...
    throw new Error(`Error Odoo ${model}.${method}: ${message}`);
  }
  return response.result;
}

export async function goToPortalSubscriptions(page, baseUrl) {
  await page.goto(buildUrl(baseUrl, "/my/subscriptions"), {
  ...
  return {
    text,
    hasSubscription:
      /plan vigente|suscripciones|suscripcion activa|activa|borrador|pendiente/i.test(
        text,
      ),
  };
}
...
export async function openSubscriptionProduct(page, baseUrl) {
  ...
  await page.goto(buildUrl(baseUrl, "/shop"), {
    waitUntil: "networkidle",
    timeout: 60000,
  });
    ...
    .evaluateAll((links) => {
      const candidates = links
        .map((link) => ({
          ...
          text: (
            link.innerText ||
            link.closest(".oe_product, .card, article")?.innerText ||
            ""
          ).toLowerCase(),
        }))
        .filter(
          (item) =>
            ...
            (item.text.includes("suscripcion") ||
              item.text.includes("mensual") ||
              item.text.includes("anual")),
        );
  ...
  await page.goto(buildUrl(baseUrl, productHref), {
    waitUntil: "networkidle",
    timeout: 60000,
  });
...
export async function attemptSubscriptionCartUpdate(page, baseUrl) {
  ...
  await page
    .locator("#add_to_cart, .js_check_product.a-submit, a.a-submit")
    .first()
    .click({ timeout: 20000 });
  await delay(3000);
  await page.goto(buildUrl(baseUrl, "/shop/cart"), {
    waitUntil: "networkidle",
    timeout: 60000,
  });
  await tomarCaptura(page, "3_carrito_suscripcion");
  const text = await pageText(page);
  const hasSubscriptionLine =
    /suscripcion mensual|suscripcion anual|suscripción mensual|suscripción anual/i.test(
      text,
    );
  const isEmpty =
    /your cart is empty|tu carrito esta vacio|tu carrito está vacío/i.test(
      text,
    );
  const hasRestriction =
    /solo puedes comprar una suscripci|ya tienes una suscripci|suscripci.n activa|suscripci.n pendiente/i.test(
      text,
    );
...
export async function assertSubscriptionFlow(page, baseUrl) {
  ...
  if (
    portalState.hasSubscription &&
    (cartState.isEmpty ||
      cartState.hasRestriction ||
      !cartState.hasSubscriptionLine)
  ) {
    console.log(
      "Resultado esperado: el usuario ya tiene suscripcion, por eso no se agrega otra al carrito.",
    );
    return;
  }
  if (!portalState.hasSubscription && cartState.hasSubscriptionLine) {
    console.log(
      "Resultado esperado: el usuario sin suscripcion puede agregar una unica suscripcion al carrito.",
    );
    return;
  }
  throw new Error(
    `Flujo de suscripcion inconsistente. Portal=${portalState.hasSubscription}, Carrito=${cartState.text.slice(0, 500)}`,
  );
}

export async function getEventLinks(page, baseUrl) {
  await page.goto(buildUrl(baseUrl, "/event"), {
    waitUntil: "networkidle",
    timeout: 60000,
  });
...
export async function openFirstAvailableEvent(page, baseUrl) {
  ...
  for (const link of links) {
    ...
    await page.goto(buildUrl(baseUrl, link.href), {
      waitUntil: "networkidle",
      timeout: 60000,
    });
    const text = await pageText(page);
    if (
      !/ya tienes un boleto|solo puedes tener un boleto|already registered|ya estas registrado|ya estás registrado/i.test(
        text,
      )
    ) {
  ...
  console.log(
    "Todos los eventos revisados parecen registrados o bloqueados para este usuario.",
  );
...
export async function startEventRegistration(page, baseUrl) {
  ...
  if (!selectedEvent) {
    console.log(
      "Resultado esperado: no hay eventos sin registro para este usuario en la pagina revisada.",
    );
  ...
  if (
    ...
    console.log(
      "Resultado esperado: el usuario ya tiene un boleto para el evento seleccionado.",
    );
    return { status: "already_registered" };
  }
  ...
  if (!(await page.locator("#registration_form").count())) {
    throw new Error(
      "No se encontro el formulario #registration_form en el detalle del evento.",
    );
  }
  ...
  await page
    .locator("#modal_ticket_registration button[type='submit']")
    .first()
    .click();
  await page.waitForSelector("#attendee_registration, input[name*='email']", {
    timeout: 20000,
  });
  ...
  if (!attendeeForms || emailInputs !== 1) {
    throw new Error(
      `El flujo actual debe generar exactamente un asistente. Formularios=${attendeeForms}, emails=${emailInputs}`,
    );
  }

  await fillIfVisible(
    page,
    "#attendee_registration input[name*='name']",
    TEST_USER.name,
  );
  await fillIfVisible(
    page,
    "#attendee_registration input[name*='email']",
    TEST_USER.email,
  );
  await fillIfVisible(
    page,
    "#attendee_registration input[name*='phone']",
    TEST_USER.phone,
  );
...
export async function finishEventRegistrationToCheckout(page) {
  ...
  await Promise.all([
    ...
    page
      .locator("#attendee_registration button[type='submit']")
      .first()
      .click(),
  ]);
  await delay(4000);
  ...
  const url = page.url();
  if (
    /confirmation|registration\/success|shop\/cart|shop\/checkout|shop\/payment|payment\/status/i.test(
      url,
    ) ||
    /gracias|thank you|pedido|confirmado|exitosa|solo puedes|ya tienes un boleto/i.test(
      text,
    )
  ) {
  ...
  throw new Error(
    `No se detecto estado valido tras registrar evento. URL=${url}; texto=${text.slice(0, 500)}`,
  );
}
...
export async function assertEventRegistrationFlow(page, baseUrl) {
  ...
  console.log(
    `Estado final del evento: ${finished.status} (${finished.url || page.url()})`,
  );
}
...
export async function assertExerciseGuideFlow(page, baseUrl) {
  await page.goto(buildUrl(baseUrl, "/exercise-guides"), {
    waitUntil: "networkidle",
    timeout: 60000,
  });
  await tomarCaptura(page, "2_guias_lista");

  const guideSearch = page
    .locator(
      "input[placeholder='Buscar guía'], input[placeholder='Buscar guia']",
    )
    .first();
  ...
  const text = await pageText(page);
  if (
    !url.includes("/exercise-guides/") ||
    !/guia|guía|spinning|ejercicio|tecnica|técnica/i.test(text)
  ) {
    throw new Error(
      `No se llego al detalle de guia esperado. URL=${url}; texto=${text.slice(0, 500)}`,
    );
  }
}
...
export async function extractFirstEventData(page, baseUrl) {
  ...
  await page.goto(buildUrl(baseUrl, links[0].href), {
    waitUntil: "networkidle",
    timeout: 60000,
  });
  ...
  return page.evaluate(() => {
    ...
    const priceMatch =
      text.match(/Precio:\s*([^\s]+\s*[\d.,]+)/i) || text.match(/\$\s*[\d.,]+/);
    const seatsMatch = text.match(/Cupos Disponibles:\s*(\d+)\s*\/\s*(\d+)/i);
    const dateMatch = text.match(
      /(\d{2}\s+[a-zA-Z]+ \s*\d{4}|\d{2}\s+[a-zA-Z]+\s+\d{4})/i,
    );
    ...
    return {
      claseGrupal: {
        nombre: heading,
        entrenador:
          clean((text.match(/Organizer\s+([^$]+?)\s+Share/i) || [])[1]) ||
          "Iron Zone",
        horario:
          clean(`${dateMatch?.[1] || ""} ${timeMatch?.[1] || ""}`) ||
          "No especificado",
        cupoMaximo: seatsMatch ? Number(seatsMatch[2]) : 0,
        cupoDisponible: seatsMatch ? Number(seatsMatch[1]) : 0,
        salaEspacio:
          clean((text.match(/Location\s+(.+?)\s+Get directions/i) || [])[1]) ||
          "No especificado",
